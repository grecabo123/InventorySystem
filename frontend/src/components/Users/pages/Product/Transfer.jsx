...
function Transfer() {
    ...
    const ListProduct = () => {
        axios.get(`/api/ProductMonitor/${localStorage.getItem('auth_id')}`).then(res => {
    ...
    const GetUpdate = (e) => {
        setVisible(true);
        setDetails({
            travel: e.currentTarget.getAttribute('data-travel_id'),
    ...
    const TransferForm = (e) => {
        ...
        axios.put(`/api/UpdateStatusProduct`,data).then(res => {
            if(res.data.status === 200) {
                toast.current.show({
                    severity: "success",
        ...
        <div>
            <Card title="Transfer Product Monitor">
                ...
                <DataTable value={Data} paginator paginatorLeft rows={9} loading={loading}>
                    ...
                    {/* <Column field='type_of_request' body={RequestType} header='Type of Request'></Column> */}
